...
第一期接入 12API 和火山方舟 Ark。后续平台由用户提供接口规范后再增加明确的适配器，不自动猜测未知接口。
...
- 12API 的三条线路和九个生图模型，以及火山方舟 Ark 的三款 Seedream 生图模型。
...
| `doubao-seedream-4-5-251128` | Ark Images | 是 | 是 | 2025-11-28 |
| `doubao-seedream-5-0-260128` | Ark Images | 是 | 是 | 2026-01-28 |
| `doubao-seedream-5-0-pro-260628` | Ark Images | 是 | 是 | 2026-06-28 |
...
火山方舟三款模型通过 `POST https://ark.cn-beijing.volces.com/api/v3/images/generations` 调用，使用 Bearer 认证和 JSON 请求体。`image` 接收一张或多张参考图，`response_format` 使用 `b64_json`，关闭水印。第一版保守限制最多 10 张参考图，并支持 2K、4K 与接口允许的自定义尺寸。
...
火山方舟第一期只有 `default` 价格分组。

| 模型 ID | 文生图 | 图生图 | 其他计费 |
| --- | ---: | ---: | --- |
| `doubao-seedream-4-5-251128` | ¥0.25/输出图 | ¥0.25/输出图 | 无 |
| `doubao-seedream-5-0-260128` | ¥0.22/输出图 | ¥0.22/输出图 | 无 |
| `doubao-seedream-5-0-pro-260628` | ¥0.30/输出图 | ¥0.30/输出图 | 第一张输入图免费，之后每张 ¥0.02 |

Seedream 5.0 Pro 的预计价格为 `输出数量 × 0.30 + max(参考图数量 - 1, 0) × 0.02`。其他两款按输出图片数量计算。
...
模型选择器由 `models.json` 渲染。第一批展示上述 12 个生图模型。
...
- 测试 12 个模型均能关联到至少一个平台映射和价格条目。
- 测试 Ark 三款模型使用独立 `ark-images` 适配器，并正确计算按输出图和额外输入图价格。
